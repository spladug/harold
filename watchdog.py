...
class UnregistrationListener(WatchdogResource):
    def _handle_request(self, request):
        tag = request.args['tag'][0]
        self.watchdog.unregister(tag)
...
def initialize(http, alerter):
    ...
    watchdog_root = resource.Resource()
    http.root.putChild("watchdog", watchdog_root)

    watchdog_root.putChild("heartbeat", HeartbeatListener(http, watchdog))
    watchdog_root.putChild("unregister", UnregistrationListener(http, watchdog))
